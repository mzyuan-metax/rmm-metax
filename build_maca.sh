...
function ensureCMakeRan {
    ...
    if (( RAN_CMAKE == 0 )); then
        ...
        cmake_maca -S "${REPODIR}"/cpp -B "${LIBRMM_BUILD_DIR}" \
...
if (( NUMARGS == 0 )) || hasArg librmm; then
    ...
    cmake_maca --build "${LIBRMM_BUILD_DIR}" -j"${PARALLEL_LEVEL}" ${VERBOSE_FLAG}
    if [[ ${INSTALL_TARGET} != "" ]]; then
        echo "installing librmm..."
        cmake_maca --build "${LIBRMM_BUILD_DIR}" --target install -v ${VERBOSE_FLAG}
    fi
fi
